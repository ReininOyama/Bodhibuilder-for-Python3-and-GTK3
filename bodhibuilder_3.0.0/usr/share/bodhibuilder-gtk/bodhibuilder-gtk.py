...
class appgui:
    ...
    def on_backup_done(self, widget, pid, error, data = None):
        if pid != 0:
            WORKDIR = self.builder.get_object("entry6").get_text()
            CUSTOMISO = self.builder.get_object("entry3").get_text()
            msg_info(_("Please Wait! When the termial Mesassge say ended.  Click on OK to return to the main menu.") 
    ...
    def on_dist_done(self, widget, pid, error, data = None):
        
        if pid != 0:
            WORKDIR = self.builder.get_object("entry6").get_text()
            CUSTOMISO = self.builder.get_object("entry3").get_text()
            msg_info(_("Please Wait! When the terminal say ended. Then, Click on OK to return to the main menu.") % ({"iso" : CUSTOMISO, "dir" : WORKDIR+'/bodhibuilder'}))
        else:
    ...
    def on_dist_cdfs_done(self, widget, pid, error, data = None):
        if pid == 0:
            WORKDIR = self.builder.get_object("entry6").get_text()
            CUSTOMISO = self.builder.get_object("entry3").get_text()
            msg_info(_("Please Wait! When the terminal says ended. Then, Click on OK to return to the main menu.") % WORKDIR+'/bodhibuilder')
        else:
            msg_error(_("The process was not started!"), self.builder.get_object("window1"))
        self.builder.get_object("notebook1").set_current_page(0)
        self.v.handler_disconnect(self.callback_id)
    ...
    def on_dist_iso_done(self, widget, pid, error, data = None):
        if pid != 0:
            WORKDIR = self.builder.get_object("entry6").get_text()
            CUSTOMISO = self.builder.get_object("entry3").get_text()
            msg_info(_("Wait! When the Terminal say ended. Then, Click on OK to return to the main menu.") % ({"iso" : CUSTOMISO, "dir" : WORKDIR+'/bodhibuilder'}))
        else:
    ...
    def on_clean_done(self, widget, pid, error, data = None):
        if pid != 0:
            msg_info(_("Wait to completed. Then, Click OK to return to the main menu."))
        else:
            msg_error(_("The process was not started!"), self.builder.get_object("window1"))
        self.builder.get_object("notebook1").set_current_page(0)
        self.v.handler_disconnect(self.callback_id)
